...
function _updateTopicCardFromPrompt(playerText:string, card:Card):UpdateCardChanges {
  ...
  for(let i = 0; i < card.keywordGoals.length; ++i) {
    ...
    const matchingWord = words.find(word => doesRhyme(word, kg.keyword) || word === kg.keyword);
    if (matchingWord) {
      didCardChange = true;
      kg.isComplete = true;
    }
